...
export const useBlurTransition = (isLoaded, container, selector) => {
    ...
    useGSAP((context, contextSafe) => {
        ...
        const handleMouseEnter = (event) => {
            ...
            elements.forEach((item) => {
                ...
                if (item !== event.target) {
                    gsap.to(item, { filter: "blur(20px)", duration: 1, ease: "power1.inOut" });       
                } 
            });
        };
        ...
        gsap.fromTo(elements, { filter: "blur(100px)" }, { filter: "blur(0px)", duration: 1, ease: "power1.inOut", stagger: 0.1 });
